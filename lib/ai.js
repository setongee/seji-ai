...
- "at 3pm" → today at 15:00 WAT
- "in 30 minutes" → now + 30 min
- Urgent with no time → now + 1 hour
- Casual no-deadline task → remind_at: null
- Always check_in: true when there's a deadline`;
...
export async function extractTasks(userMessage, now, { history = [], userName = null, pendingTasks = [] } = {}) {
  ...
  if (pendingTasks.length > 0) {
    const list = pendingTasks
      .map((t, i) => `${i + 1}. ${t.title}${t.remind_at ? ` (due ${t.remind_at})` : ""}`)
      .join("\n");
